...
def lrt_combo_name(lrts: list[str]) -> str:
    parts = [lrt.lower() for lrt in LRT_ORDER if lrt in lrts]
    ...
    for prt in PRTS:
        for arm_obrm in ARM_OBRM_STATES:
            for bits in itertools.product([False, True], repeat=len(LRT_BINARY)):
                lrts = arm_obrm + [lrt for lrt, b in zip(LRT_BINARY, bits) if b]

                for tech_bits in range(1 << len(TECH_FIELDS)):  # 0..63
                    expensive = [
                        TECH_FIELDS[i] for i in range(len(TECH_FIELDS)) if tech_bits & (1 << i)
                    ]
